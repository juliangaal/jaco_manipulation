...
bool GraspPoseGenerator::serverCB(jaco_manipulation::GenerateGraspPosesRequest &request,
                                  jaco_manipulation::GenerateGraspPosesResponse &response) {

  geometry_msgs::PointStamped pt = request.object_location;
  ...
  target_pose_top_.header = request.object_location.header;
  target_pose_1_.header = request.object_location.header;
  target_pose_2_.header = request.object_location.header;
  target_pose_3_.header = request.object_location.header;
  ...
  response.grasp_poses.resize(4);
  response.pregrasp_poses.resize(4);

  // First the grasp pose. Then the pregrasp pose
  target_pose_top_.pose.position.z += 0.02;
  target_pose_top_.pose.position.y -= 0.05;
  response.grasp_poses[0] = target_pose_top_;
  target_pose_top_.pose.position.z += 0.03;
  response.pregrasp_poses[0] = target_pose_top_;


  // First the grasp pose. Then the pregrasp pose
  target_pose_1_.pose.position.y -= 0.020;
  response.grasp_poses[1] = target_pose_1_;
  target_pose_1_.pose.position.y += 0.07;
  response.pregrasp_poses[1] = target_pose_1_;

  // First the grasp pose. Then the pregrasp pose
  target_pose_2_.pose.position.y -= 0.020;
  target_pose_2_.pose.position.x += 0.020;
  response.grasp_poses[2] = target_pose_2_;
  target_pose_2_.pose.position.y += 0.070;
  target_pose_2_.pose.position.x -= 0.070;
  response.pregrasp_poses[2] = target_pose_2_;

  // First the grasp pose. Then the pregrasp pose
  target_pose_3_.pose.position.y -= 0.022;
  target_pose_3_.pose.position.x += 0.022;
  response.grasp_poses[3] = target_pose_3_;
  target_pose_3_.pose.position.y += 0.070;
  target_pose_3_.pose.position.x -= 0.070;
  response.pregrasp_poses[3] = target_pose_3_;

  response.grasp_poses[0].header.frame_id = "root";
  response.grasp_poses[1].header.frame_id = "root";
  response.grasp_poses[2].header.frame_id = "root";
  response.grasp_poses[3].header.frame_id = "root";

  response.pregrasp_poses[0].header.frame_id = "root";
  response.pregrasp_poses[1].header.frame_id = "root";
  response.pregrasp_poses[2].header.frame_id = "root";
  response.pregrasp_poses[3].header.frame_id = "root";
